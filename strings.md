...
Laravel zawiera różnorodne funkcje do manipulowania wartościami łańcuchów znaków. Wiele z tych funkcji jest używanych przez sam framework; jednak możesz swobodnie używać ich we własnych aplikacjach, jeśli uznasz je za wygodne.
...
Funkcja `__` tłumaczy podany łańcuch tłumaczenia lub klucz tłumaczenia używając twoich [plików językowych](/docs/{{version}}/localization):
...
Jeśli podany łańcuch tłumaczenia lub klucz nie istnieje, funkcja `__` zwróci podaną wartość. Więc, używając powyższego przykładu, funkcja `__` zwróciłaby `messages.welcome`, jeśli ten klucz tłumaczenia nie istnieje.

<a name="method-class-basename"></a>
#### `class_basename()` {.collection-method}

Funkcja `class_basename` zwraca nazwę klasy podanej klasy z usunietnymi przestrzeniami nazw klasy:
...
Funkcja `e` uruchamia funkcję PHP `htmlspecialchars` z opcją `double_encode` ustawioną domyślnie na `true`:
...
Funkcja `preg_replace_array` zamienia dany wzór w łańcuchu sekwencyjnie używając tablicy:
...
Metoda `Str::after` zwraca wszystko po podanej wartości w łańcuchu. Cały łańcuch zostanie zwrócony, jeśli wartość nie istnieje w łańcuchu:
...
Metoda `Str::afterLast` zwraca wszystko po ostatnim wystąpieniu podanej wartości w łańcuchu. Cały łańcuch zostanie zwrócony, jeśli wartość nie istnieje w łańcuchu:
...
Metoda `Str::apa` konwertuje podany łańcuch na tytuł zgodnie z [wytycznymi APA](https://apastyle.apa.org/style-grammar-guidelines/capitalization/title-case):
...
Metoda `Str::ascii` spróbuje transliterować łańcuch na wartość ASCII:
...
Metoda `Str::before` zwraca wszystko przed podaną wartością w łańcuchu:
...
Metoda `Str::beforeLast` zwraca wszystko przed ostatnim wystąpieniem podanej wartości w łańcuchu:
...
Płynne łańcuchy znaków zapewniają bardziej płynny, obiektowy interfejs do pracy z wartościami łańcuchów znaków, pozwalając łączyć wiele operacji na łańcuchach razem używając bardziej czytelnej składni w porównaniu z tradycyjnymi operacjami na łańcuchach.

<a name="method-fluent-str-after"></a>
#### `after` {.collection-method}

Metoda `after` zwraca wszystko po podanej wartości w łańcuchu. Cały łańcuch zostanie zwrócony, jeśli wartość nie istnieje w łańcuchu:
